...
function ValidatedOARecordsFromTXTRecordsWithOpenAliasPrefix (domain, records, dnssec_used, secured, dnssec_fail_reason, openAliasPrefix) {
  ...
  if (dnssec_used) {
    if (secured) {
      ...
      throw Error('DNSSEC validation failed for ' + domain + ': ' + dnssec_fail_reason)
    }
  } else {
  ...
  if (oaRecords.length === 0) {
    throw Error('No valid OpenAlias records with prefix ' + openAliasPrefix + ' found for: ' + domain)
  }
  if (oaRecords.length !== 1) {
    throw Error('Multiple addresses found for given domain: ' + domain)
  }
